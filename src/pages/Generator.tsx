...
import { Download, Upload, Wand2, Video, Image as ImageIcon, Loader2, AlertCircle, Zap } from "lucide-react"
...
const Generator = () => {
  ...
  // Update credits when image job is completed
  useEffect(() => {
    if (imageJobStatus?.status === 'done' && user) {
      // Refetch credits after image generation/regeneration
      const fetchUpdatedCredits = async () => {
        try {
          ...
          if (!error && data) {
            ...
            // Show toast notification for credit deduction
            if (isRegeneratingRef.current) {
              toast({
                title: "Credits Updated",
                description: `5 credits deducted for image regeneration. You now have ${newCredits} credits remaining.`,
                duration: 3000,
              });
            } else {
              toast({
                title: "Credits Updated",
                description: `5 credits deducted for image generation. You now have ${newCredits} credits remaining.`,
                duration: 3000,
              });
            }
          }
  ...
  }, [imageJobStatus?.status, user, toast]);
  ...
  // Update credits when video job is completed
  useEffect(() => {
    if (videoJobStatus?.status === 'done' && user) {
      // Refetch credits after video generation
      const fetchUpdatedCredits = async () => {
        try {
          const { data, error } = await supabase
            .from('profiles')
          ...
          if (!error && data) {
            ...
            toast({
              title: "Credits Updated",
              description: `50 credits deducted for video generation. You now have ${newCredits} credits remaining.`,
              duration: 3000,
            });
          }
        } catch (error) {
  ...
  }, [videoJobStatus?.status, user, toast]);
              ...
              <Card className="shadow-soft border-0">
                ...
                <CardContent className="space-y-6">
                  ...
                  <Button
                    ...
                    disabled={isGeneratingImage || (credits !== null && credits < 5)}
            ...
            <div className="space-y-6">
              ...
              <div className="flex justify-end gap-3">
                <Button
                  ...
                  disabled={!imageJobId || isRegeneratingImage || (credits !== null && credits < 5)}
                  variant="outline"
                ...
                {credits !== null && credits < 5 && (
                  <p className="text-destructive text-sm mt-2">
                    Insufficient credits. You need 5 credits to regenerate an image.
                  </p>
                )}
                <Button
                  onClick={handleGenerateVideo}
                  disabled={!generatedPhoto || isGeneratingVideo || ((videoJobStatus?.status === 'queued' || videoJobStatus?.status === 'processing') && !generatedVideo) || (credits !== null && credits < 50)}
                ...
                {credits !== null && credits < 50 && (
                  <p className="text-destructive text-sm mt-2">
                    Insufficient credits. You need 50 credits to generate a video.
                  </p>
                )}
              </div>
